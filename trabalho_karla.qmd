---
title: "Análise da distribuição de eventos adversos de interesse especial graves na vacinação contra a COVID-19 no Brasil e da cobertura vacinal para o esquema primário no estado do Rio de Janeiro"
...
par(mfrow=c(1,2))
boxplot(dft.ts ~ trunc(time(dft.ts)), xlab = "", ylab ="EAIE/100.000 internações", col = "burlywood3",  cex.lab=0.5, cex.axis=0.5, main = "(a)", cex.main=0.8)
boxplot(dft.ts ~ cycle(dft.ts), names = month.abb, xlab = "", col = "aquamarine3", ylab = "",cex.lab=0.5, cex.axis=0.5, main = "(b)", cex.main=0.8) 
dev.off()
...
- Verificar a importância do Índice de Desenvolvimento Humano (IDH) para explicar os nívels de cobertura vacinal no esquema primário para COVID-19 nos municípios do Rio de Janeiro.
...
library(cowplot)
map.res <- cowplot::plot_grid(res.gwr, viol, rows = 2)
ggsave("res_gwr_rj.jpg", map.res, dpi=300)

```

A seguir, constam os resultados para a modelagem da cobertura vacinal contra a COVID-19 (doses 1 e 2) em função do IDH.A modelagem GWR ajusta uma regressão para cada ponto com a ponderação das demais observações em função da distância desse ponto. As variações espaciais são modeladas de forma contínua, com parâmetros variando no espaço. Assim, cada município possui seu próprio coeficiente que encontra-se representado espacialmente na @fig-Figura14 (a). No mapa temático, os maiores valores de coeficientes são marcados pelas tonalidade de rosa/laranja e os menores nos tons de verde. 

Os coeficientes significativos (@fig-Figura14 (b)) indicam os locais onde a variável IDH é uma forte preditora da cobertura vacinal para COVID-19, que são Rio de Janeiro, São João de Meriti e Engenheiro Paulo Frontin.

```{r Figura14, include=TRUE}
#| label: fig-Figura14
#| fig-cap: "Resultados do modelo GWR. (a) Distribuição dos coeficientes do modelo GWR para a variável IDH. (b) Coeficientes para a variável IDH significativos segundo o teste de Wald."
#| out-width: "100%"
...
De modo geral, o ajuste do modelo é fraco (Quasi-global R2: 0.35). O mapa temático para o R2 local consta na @fig-Figura15. Observa-se melhores ajustes nas áreas destacadas em rosa mais claro. Os valores preditos e observados do modelo GWR ajustado encontra-se apresentado na @fig-Figura16.

```{r Figura15, include=TRUE}
#| label: fig-Figura15
#| fig-cap: "Resultados do modelo GWR. Mapa do coeficientes de determinação locais (R2 local)."
#| out-width: "80%"
...
Quanto à adequação do modelo GWR, não se observa autocorrelação espacial nos resíduos (I= -0.01, p-valor=0.50) que parecem estar distribuídos espacialmente de forma aleatória (@fig-Figura17).
