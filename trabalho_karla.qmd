---
title: "Análise temporal dos eventos adversos de interesse especial graves na vacinação contra a COVID-19 no Brasil e Análise espacial da cobertura vacinal para o esquema primário no estado do Rio de Janeiro"
...
-   Descrever a ocorrência de internações hospitalares no SUS correspondentes aos EAIE graves no período anterior à vacinação e avaliar os efeitos da introdução da vacinação sobre a distribuição temporal desses eventos.
...
#sazonalidade
saz <- gg_subseries(dft.tb, xlab ="EAIE/100.000 internações", ylab="") 


#tendencia e sazonalidade
jpeg(file="sazon_tenden.jpg", width = 15, height = 10, units = "cm", pointsize = 12,res = 600, quality = 85) 
par(mfrow=c(1,2))
boxplot(dft.ts ~ trunc(time(dft.ts)), xlab = "", ylab ="EAIE/100.000 internações", col = "burlywood3",  cex.lab=0.7, cex.axis=0.7, main = "(a)", cex.main=0.8)
boxplot(dft.ts ~ cycle(dft.ts), names = month.abb, xlab = "", col = "aquamarine3", ylab = "",cex.lab=0.7, cex.axis=0.7, main = "(b)", cex.main=0.8) 
dev.off()
...
No período de janeiro de 2015 a dezembro de 2017 ocorreram 3.735.180 EAIE graves em um total de 3.627.942 internações. A idade mediana na amostra é de 63 anos (IIQ: 51-76), com 60% do pacientes com idade superior ou igual a 60 anos e 53,7% do sexo masculino. O período de análise compreende 96 meses sendo o valor mínimo de 4.060 EAIE por 100.000 internações observado em março de 2015 e o valor máximo em agosto de 2020 (5.760/100.000), com média de 4.892/100.000.

A @fig-Figura1 apresenta a taxa mensal de EAIE graves para a vacinação contra a COVID-19 por 100.000 internações no período de análise. A série apresenta componente de sazonalidade com a maior parte dos casos ocorrendo no fim de cada ano, com exceção dos anos de 2020 e 2021 que apresentam pico de casos no meio do ano, conforme se verifica com maior clareza na @fig-Figura2. É também notável a presença de tendência com evidente aumento dos casos no período observado (@fig-Figura2).
...
#| label: fig-Figura2
#| fig-cap: "Taxa de eventos adversos de interesse especial graves para a vacinação contra a COVID-19 por 100.000 internações no SUS no período de 2015 a 2022 por anos e meses da série - (a) tendência (b) sazonalidade."
#| out-width: "100%"

knitr::include_graphics("sazon_tenden.jpg", dpi=600)

```

A @fig-Figura3 apresenta a decomposição da série temporal da taxa mensal de EAIE graves pelo método STL (*Seasonal and Trend decomposition using Loess*). De acordo com as métricas de força do método, a tendência é um componente forte (0,94) e a sazonalidade é um componente de força intermediária (0,619). No componente gráfico da tendência, verifica-se o aumento da taxa de EAIE ao longo da série, com uma estabilização nos últimos anos. A @fig-Figura4 mostra a série original e a série recomposta pelo método STL.
...
#| label: fig-Figura7
#| fig-cap: "Análise dos resíduos para o modelo ARIMA(2,1,0)(0,1,1)$_{12}$ do contrafactual."
#| out-width: "80%"
...
#| label: fig-Figura8
#| fig-cap: "Gráfico de densidade dos resíduos para os modelos ARIMA(2,1,0)(0,1,1)$_{12}$ - série completa e contrafactual."
#| out-width: "80%"
...
-   Verificar a importância do Índice de Desenvolvimento Humano (IDH) para explicar os níveis de cobertura vacinal no esquema primário para COVID-19 nos municípios do Rio de Janeiro.

## **Métodos**

Foi realizado um estudo ecológico a partir de dados que foram obtidos na página do Painel de Cobertura Vacinal COVID do Ministério da Saúde [@coberturavacinalms] relativos a dezembro de 2023. Esse estudo utilizou dados de cobertura por residência, que consideram as informações do endereço cadastrado no Cartão Nacional de Saúde (CNS) do cidadão [@SEI/MS0036724379].
...
Por fim, foi feita modelagem para avaliar a relação entre a cobertura vacinal para COVID-19 (doses 1 e 2) e o IDH dos municípios, considerando a interferência causada pela dependência espacial. Para isso foi utilizada a regressão ponderada espacialmente (*GWR - geographically weighted regression*), um tipo de modelo com variação local (não estacionaridade). Essa opção foi feita pelo fato do modelo CAR (*Conditional Autoregressive Modeling*) ter apresentado autocorrelação nos resíduos. Os dados do IDH do ano de 2010 foram obtidos na página eletrônica do IBGE.
...
Para identificar a estrutura de correlação espacial entre os municípios, foi obtida uma matriz de vizinhança por contiguidade. O mapa de conectividade por contiguidade dos municípios do Rio de Janeiro é apresentado na @fig-Figura10.
...
A @fig-Figura12 ilustra os municípios que tiveram os p-valores mais significativos no Moran Local. O mapa temático Moran Local (Lisa Map) para a cobertura vacinal consta na @fig-Figura13. Esses resultados mostram que algumas áreas apresentam alta correlação espacial. O aglomerado de muncípios em vermelho sinaliza tendência de aumento da cobertura vacinal na medida que os vizinhos aumentam (Rio das Flores, Paraíba do Sul, Paty do Alferes, Vassouras, Engenheiro de Paulo Frontin, Mendes, Barra do Piraí, Volta Redonda e Barra Mansa). Também, observa-se que a capital do Rio de Janeiro tem uma cobertura relativamente mais baixa que seus vizinhos, mesmo tendo atingido a meta de cobertura (96,1%).

```{r Figura12, include=TRUE}
#| label: fig-Figura12
#| fig-cap: " Municípios do Rio de Janeiro com p-valores significativos no Moran Local para a cobertura vacinal das doses 1 e 2 contra a COVID-19."
#| out-width: "80%"
...
library(spatialreg)
#usar cobertura nao multiplicada por 100
cob.car <- spatialreg::spautolm(cobertura_vacinal_2_doses_percent ~ X2010, data = rj.sf2,listw = pesos.vizrj, family = "CAR")
...
#entao vamos fazerum modelo que considera isso
...
GWRbanda <- gwr.sel(cobertura_vacinal_2_doses_percent ~ X2010, data = rj.sf2,coords = coordenadas, adapt = T)

#modelo gwr
cob.gwr <- gwr(cobertura_vacinal_2_doses_percent ~ X2010, data = rj.sf2,
    coords = coordenadas, adapt = GWRbanda, hatmatrix = TRUE, se.fit = TRUE)

cob.gwr
#Quasi-global R2: 0.354926

results <- as.data.frame(cob.gwr$SDF)
head(results)

#autocorrelacao nos residuos
results$residuos <- rj.sf2$cobertura_vacinal_2_doses_percent - results$pred
...
map.gwr <- gridExtra::grid.arrange(map.idh, map.wald, nrow = 2)
ggsave("gwr_rj.jpg",map.gwr, dpi=300)

#coeficientes de determinacao regionalizados

r2 <- ggplot(rj.sf2) + 
  geom_sf(aes(fill = localR2), color = "black",
    size = 0.2) + scale_fill_gradientn(colours = terrain.colors(10)) +
   # ggtitle("R² local") + 
  theme_void()
#ajuste fraco, IDH explica mal a cobertura
ggsave("r2_rj.jpg",r2, dpi=300)

#valores ajustados versus predito
# quantile(log(PE2$inc+1),seq(0,1,by=0.2))
breaks <- c(0, quantile(rj.sf2$cobertura_vacinal_2_doses_percent))

labels <- c("(0-45.9%]", "(45.9%-73.8%]", "(73.8%-80.5%]",
    "(80.5%-92.1%]", "(92.1%-122.3%]")  

rj.sf2$brks <- cut(rj.sf2$cobertura_vacinal_2_doses_percent, include.lowest = FALSE,right = TRUE, breaks = breaks, labels = labels)

bruta <- ggplot(rj.sf2) + 
  geom_sf(aes(fill = brks), color = "black",
    size = 0.2) + ggtitle("Observado") + scale_fill_discrete_sequential(palette = "Heat2",
    c1 = 80, c2 = 30, l1 = 30, l2 = 90, p1 = 0.2, p2 = 1.5,
    na.value = "grey75", drop = FALSE, name = "") +
    theme_void()

rj.sf2$brks.gwr <- cut(results$pred, lowest = FALSE, right = TRUE,breaks = breaks, labels = labels)

pred.gwr <- ggplot(rj.sf2) + 
  geom_sf(aes(fill = brks.gwr),
...
pred_aj <- ggpubr::ggarrange(bruta, pred.gwr,
    ncol = 1, common.legend = TRUE, legend = "right")
ggsave("pred_aj_rj.jpg",pred_aj, dpi=300)

#distribuicao dos residuos

#res_pad <- results$residuos/sd(results$residuos)

vioplot::vioplot(results$residuos, col = "darkgreen", names = "GWR")

viol <- ggplot(results, aes(x= "", y=residuos)) +
  geom_violin( width=1.4, fill = "darkgreen") +
...
#breaks2 <- c(-4, -1, 0, 1.2, 3.6)
#labels2 <- c("< -1", "-1 a 0", "0 a 1.2", "> 1.2")
breaks2 <- c(-0.4, -0.1, 0, 0.1, 0.35)
labels2 <- c("< -0.1", "-0.1 a 0", "0 a 0.1", "> 0.3")

brks.res.gwr <- cut(results$residuos, include.lowest = TRUE,
    right = TRUE, breaks = breaks2, labels = labels2)

res.gwr <- ggplot(rj.sf2) + 
  geom_sf(aes(fill = brks.res.gwr),
    color = "black", size = 0.2) + 
  ggtitle("(a)") +
...
#violino e mapa de residuos
library(cowplot)
map.res <- cowplot::plot_grid(res.gwr, viol, cols = 2)
ggsave("res_gwr_rj.jpg", map.res, dpi=300)

```

Na sequência, constam os resultados para a modelagem da cobertura vacinal contra a COVID-19 (doses 1 e 2) em função do IDH. A modelagem GWR ajusta uma regressão para cada ponto com a ponderação das demais observações em função da distância desse ponto. As variações espaciais são modeladas de forma contínua, com parâmetros variando no espaço. Assim, cada município possui seu próprio coeficiente que encontra-se representado espacialmente na @fig-Figura14 (a). No mapa temático, os maiores valores de coeficientes são marcados pelas tonalidades de rosa/laranja e os menores nos tons de verde. Os coeficientes significativos (@fig-Figura14 (b)) indicam os locais onde a variável IDH é uma forte preditora da cobertura vacinal para COVID-19, que são Rio de Janeiro, São João de Meriti e Engenheiro Paulo Frontin.

De modo geral, o ajuste do modelo é fraco (Quasi-global R^2^: 0.35), O mapa temático para o R^2^ local consta na @fig-Figura15. Observa-se melhores ajustes nas áreas destacadas em rosa mais claro. Os valores observados e preditos indicam a baixa confiabilidade do modelo ajustado GWR (@fig-Figura16). 

```{r Figura14, include=TRUE}
#| label: fig-Figura14
#| fig-cap: "Resultados do modelo GWR. (a) Distribuição dos coeficientes do modelo GWR para a variável IDH. (b) Coeficientes significativos para a variável IDH segundo o teste de Wald."
#| out-width: "100%"

knitr::include_graphics("gwr_rj.jpg", dpi=600)

```

```{r Figura15, include=TRUE}
#| label: fig-Figura15
#| fig-cap: "Resultados do modelo GWR. Mapa do coeficientes de determinação locais (R$^2$ local)."
#| out-width: "80%"

knitr::include_graphics("r2_rj.jpg", dpi=600)

```

```{r Figura16, include=TRUE}
#| label: fig-Figura16
#| fig-cap: "Valores observadose e preditos pelo modelo GWR."
#| out-width: "100%"

knitr::include_graphics("pred_aj_rj.jpg", dpi=600)

```

Quanto à adequação do modelo GWR, não se observa autocorrelação espacial nos resíduos (I=-0.01, p-valor=0.50) que parecem estar distribuídos espacialmente de forma aleatória no mapa (@fig-Figura17).

```{r Figura17, include=TRUE}
#| label: fig-Figura17
#| fig-cap: "(a) Mapa dos resíduos do modelo GWR. (b) Gráfico violino da distribuição dos resíduos."
#| out-width: "90%"

knitr::include_graphics("res_gwr_rj.jpg", dpi=600)

```

## **Discussão**

A análise espacial da cobertura vacinal em nível municipal apresenta importantes limitações, isto porque nas faixas etárias mais jovens houve importantes fluxos de deslocamento para vacinação devido a calendários divergentes em municípios vizinhos. Esses deslocamentos chegaram a resultar em até 20% do total de doses aplicadas fora do município de residência dos vacinados [@monitoracovid-19-icict/fiocruz2021]. Assim, pode-se explicar o fato de alguns municípios do Rio de Janeiro apresentarem coberturas vacinais acima de 100%.

De modo geral, a cobertura vacinal para as doses 1 e 2 das vacinas para a COVID-19 aparentemente possui baixa correlação espacial quando consideramos a divisão de áreas segundo os municípios do estado do Rio de Janeiro. Foram identificados poucos clusters onde essa associação é mais intensa. A relação entre IDH e cobertura vacinal já apontada no nível de regiões geográficas no Brasil[@fiocruz] não foi verificada nesse estudo. A relação se mostrou significativa apenas em dois dos 92 municípios do estado. Cabe ressaltar que esses coeficentes não foram interpretados devido à baixa confiabilidade do modelo. Para dar continuidade a essa análise, seria fundamental buscar outras variáveis que ajudem a explicar a cobertura vacinal para as primeiras doses das vacinas contra a COVID-19. 
...
```{r cids, include = TRUE}
library(kableExtra)
aesi <- read.csv("/Users/karlaferreira/Documents/Ensp 2023/Seminario1/AESI.csv")
cid <- as.data.frame(unique(aesi$CID10))
ev_cid <- merge(cid, aesi[,c("CID10", "BC")], 
                by.x = "unique(aesi$CID10)",
                by.y = "CID10", all.x = TRUE)

ev_cid <- unique(ev_cid)
names(ev_cid) <- c("CID10","EAIE")

ev_cid2 <- aggregate(CID10 ~ EAIE, data= ev_cid, 
                    FUN = function(x) paste(x, collapse = ", "))

knitr::kable(ev_cid2, align = "ll", format = "latex", booktabs = TRUE, caption ="Subcategorias CID 10 segundo Evento adverso de interesse especial (EAIE).") |>
