...
#cobertura vacinal esquema primário por UF
#em 21/12/23
#Ministério da Saúde - Cobertura Vacinal COVID-19
cob <- readxl::read_xlsx("cob_UF.xlsx")
cob <- cob |> janitor::clean_names()
names(cob)

#juntar os dados de cobertura e shape UF
uf.sf <- uf.sf |> 
    mutate(uf_residencia = SIGLA_UF)  |> 
    inner_join(cob, by = "uf_residencia")

uf.sf <- uf.sf |> 
  mutate(cobertura = round(cobertura_vacinal_2_doses_percent*100, digits = 1))

g1 <- ggplot(uf.sf) + 
    geom_sf(aes(fill = cut_interval(cobertura,5)), size = 0.2, legend_size = 2) +       colorspace::scale_fill_discrete_sequential(palette = "Reds",
name = "cobertura %") +
...
g2 <- ggplot(uf.sf, aes(cobertura)) + geom_density(fill = "red",
    alpha = 0.2) + geom_vline(xintercept = quantile(uf.sf$cobertura,
    probs = seq(0, 1, by = 0.2)), color = "red", linetype = "dashed") +
    labs(x= "cobertura %", y = "densidade") +
    theme(text = element_text(size = 7)) +
    theme(plot.margin = margin(3, 1, 1, 1, "cm"))

tema <- ggpubr::ggarrange(g1, g2, nrow = 1, widths = c(6.5,3.5))
...
viz.sf <- st_set_crs(viz.sf, st_crs(uf.sf))

# Plotando o mapa de conectividade por
# contiguidade
ggplot() + geom_sf(data = uf.sf, fill = "salmon", size = 0.2,
...
moran.test(uf.sf$cobertura, pesos.viz)

#correlograma
correl <- sp.correlogram(viz, uf.sf$cobertura, order = 4, method = "I")
correl
#Correlograma
plot(correl)

#uf com p-valores mais significativos no Moran Local
uf.sf$pval <- localmoran(uf.sf$cobertura, pesos.viz)[, 5]

plot_pv <- tm_shape(uf.sf) + 
...
resI <- localmoran.sad(lm(uf.sf$cobertura ~ 1), 1:length(viz),
    viz, style = "W")
summary(resI)[1:5, ]

uf.sf$MoranLocal <- summary(resI)[, 1]
