...
A variável de desfecho foi definida como o total de EAIE graves por 100.000 internações/mês. O início da intervenção foi janeiro de 2021, quando foi iniciada a campanha de vacinação no Brasil. No modelo foram incluídas covariáveis para indicar o início da pandemia, bem como o início da intervenção na série temporal [@schaffer2021].   
...
df <-df |> mutate (regiao0 = substr(as.character(UF_ZI), 1,2))
...
plot1 <- autoplot(dft.ts, ylab = "EAIE/100.000 internações")
...
library(Kendall)
#jpeg(file="figures/total_ts.jpg", width = 15, height = 15, units = "cm", pointsize = 12,
#     res = 600, quality = 85)
...
# mape arima pós intervenção: 5.949595

# Plot
jpeg(file="arima.jpg", width = 15, height = 10, units = "cm", pointsize = 12,
    res = 600, quality = 85)
...
No período de janeiro de 2015 a dezembro de 2017 ocorreram 3.735.180 EAIE graves em um total de 3.627.942 internações (1,02 EAIE por internação). A idade mediana na amostra é de 64 anos (IQT: 52-74), com 60% do pacientes com idade superior ou igual a 60 anos e 52,7% do sexo masculino. O período de análise compreende 96 meses sendo o valor mínimo de 4.060 EAIE por 100.000 internações observado em março de 2015 e o valor máximo em agosto de 2020 (5.760/100.000), com média de 4.892.
...
A @fig-Figura4 compara as taxas de EAIE observadas na série com os valores preditos (na cor vermelha) pelo modelo ARIMA para o período posterior ao início da vacinação. Pelo gráfico verifica-se que não houve aumento de internações por EAIE em relação ao que seria esperado para o período. O efeito imediato da vacinação a partir de fevereiro de 2023 foide -194,4 EAIE por 100.000 internações (IC 95% -404,2;15,4), seguido de uma diminuição sustentada de 1,4/100.000 (IC95%: -32,6;29,9) por mês até dezembro de 2022. De acordo com o contrafactual definido pelo modelo ARIMA, não há diferença estatisticamente significante entre os períodos pré e pós vacinação com relação à hospitalização pelos EAIE. 

Esse resultado inicial reforça a segurança das vacinas contra a COVID-19 no cenário nacional, no entanto ainda seriam necessárias análises por subgrupos (sexo, faixa etária) para melhor caracterização da incidência EAIE com dados do SUS.

Os resultados relativos à qualidade do ajuste do modelo encontram-se .....

```{r Figura4, include=TRUE}
#| label: fig-Figura4
#| fig-cap: "Valores observados e valores preditos com base em um modelo ARIMA para a taxa de eventos adversos de interesse especial graves no SUS após o início da vacinação contra a COVID-19."
#| out-width: "80%"
